...
class SecureEscrowService:
    ...
    def accept_offer(user_accepter, offer_id, beneficiary_data=None):
        ...
        if offer.status != 'OPEN':
            raise ValidationError(f"Offer not available (Status: {offer.status})")
            
        if offer.user == user_accepter:
            raise ValidationError("Cannot accept your own offer")

        if user_accepter.kyc_status != 'verified':
            raise ValidationError("KYC required to accept an offer.")
            
        # A1 = offer.user (Initial seller)
        # A2 = user_accepter (Buyer)
        
        # 1. Security validation: Beneficiary B1's currency (the one receiving currency_sell)
        ...
        with db_transaction.atomic():
            # 1. Locking + Balance check A1 (Optimistic + DB Lock)
